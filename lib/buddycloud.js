...
Buddycloud.prototype._events = {
    ...
    'xmpp.buddycloud.http.approve': 'approveRequest',
...
Buddycloud.prototype.handles = function(stanza) {
    if (this.channelServer && (stanza.attrs.from === this.channelServer)) return true
...
Buddycloud.prototype.discoverMediaServer = function(data, callback) {
    ...
    if (typeof callback !== 'function')
        return this._clientError('Missing callback', data)
    if (!data || !data.domain)
        return this._clientError('Missing \'domain\' key', data, callback)
