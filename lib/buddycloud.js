...
Buddycloud.prototype.createNewNode = function(data, callback) {
    ...
    if (!this._checkNode(data, callback)) return
...
Buddycloud.prototype.getNodeAffiliations = function(data, callback) {
    ...
    if (!data.channel) {
        delete data.node
    } else if (!this._checkNode(data, callback)) {
        return
    }
    var self = this
    this.getAffiliations(data, function(error, affiliations, rsm) {
        if (error) return callback(error)
        affiliations.forEach(function(affiliation) {
            self._parseNode(affiliation.node, affiliation)
        })
        callback(null, affiliations, rsm)
    })
}
...
Buddycloud.prototype._checkNode = function(data, callback) {
    if (!data.channel)
        return this._clientError('Missing \'channel\' key', data, callback)
    this._buildNode(data)
    return true
}

Buddycloud.prototype._buildNode = function(data) {
    var channel = data.channel
    var node    = data.node || 'posts'
    delete data.channel
    delete data.node
    data.node = '/users/' + channel + '/' + node
    return true
}

Buddycloud.prototype._parseNode = function(node, data) {
    var nodeParts = node.split('/')
    data.channel = nodeParts[2]
    data.node = nodeParts[3]
}
...
Buddycloud.prototype._purgeNodeNotification = function(stanza, purge) {
    var data = {}
    this._parseNode(purge.attrs.node, data)
