...
Buddycloud.prototype.registerEvents = function() {
    ...
    this.socket.on('xmpp.buddycloud.search.get', function(data, callback) {
        if (!self._checkCall(data, callback)) return
        self.search.getSearchFields(data, callback)
    })
    this.socket.on('xmpp.buddycloud.search.do', function(data, callback) {
        if (!self._checkCall(data, callback)) return
        self.search.performSearch(data, function(error, data) {
            if (error) return callback(error)
            var counter = 0
